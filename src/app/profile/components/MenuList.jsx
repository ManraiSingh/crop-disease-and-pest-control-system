    ...
    <div className="mx-4 overflow-hidden rounded-2xl border border-solid border-gray-100 bg-white">
      {ITEMS.map((item, i) => (
        ...
        >
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm font-semibold text-black">{t(item.title)}</p>
            <p className="text-[11px] text-gray-500">{t(item.subtitle)}</p>
          </div>
      ...
      >
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[11px] text-gray-500">{t('profile.logOutSub')}</p>
        </div>
        <Icon name="chevronRight" className="h-4 w-4 shrink-0 text-gray-300" />
      </button>
    </div>
