...
const STATS = [
  { key: 'area', icon: 'sprout', label: 'home.totalArea', value: '125 ha', sub: 'home.areaThisMonth' },
  { key: 'crops', icon: 'wheat', label: 'home.cropsLabel', value: '4', sub: 'home.activeCrops' },
  { key: 'health', icon: 'heart', label: 'home.fieldHealth', value: '85%', sub: 'home.goodCondition' },
  { key: 'yield', icon: 'trendingUp', label: 'home.estimatedYield', value: '8.4 t/ha', sub: 'home.yieldVsSeason' },
]
  ...
  return (
    <section className={`${GLASS_SURFACE_STRONG} rounded-3xl p-4`}>
      ...
      <div className="relative grid grid-cols-2 gap-3">
        {STATS.map((stat) => (
          <div key={stat.key} className={`${GLASS_INSET} flex items-center justify-between gap-2 p-3`}>
            <div className="min-w-0">
              <p className="truncate text-[10px] font-medium text-white/60">{t(stat.label)}</p>
              <p className="mt-1 text-base font-bold text-white">{stat.value}</p>
              <p className="truncate text-[9px] text-white/50">{t(stat.sub)}</p>
            </div>
